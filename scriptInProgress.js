...
async function ScanFiles(modelPath) {
  let files = [];
  try {
    ...
    console.log(replacedFiles, "total read" + modelCounter);
...
async function bigImage(modelName, imagePath, smallPreview, socket) {
  ...
  for (const model of modelName) {
    ...
    try {
      await page.waitForSelector(".model-image", { timeout: 10000 });
    } catch (err) {
  ...
  console.log("Big previews done! open2");
  return result
}
...
module.exports = { ScanFiles, smallImage, bigImage };
